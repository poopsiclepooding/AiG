...
def train(audio_model, train_loader, test_loader, args):
    ...
    while epoch < args.n_epochs + 1:
        ...
        # Check initialization for cls_token
        #print("cls token", audio_model.v.cls_token)
        for name, module in audio_model.named_modules():
          for param_name, param in module.named_parameters():
              print(f"Layer: {name}, Parameter: {param_name}, dtype: {param.dtype}")

          for buffer_name, buffer in module.named_buffers():
              print(f"Layer: {name}, Buffer: {buffer_name}, dtype: {buffer.dtype}")
        ...
        for i, (audio_input, labels) in enumerate(train_loader):
            ...
            for name, param in audio_model.named_parameters():
              if param.grad is not None and (torch.isnan(param.grad).any() or torch.isinf(param.grad).any()):
                  print(f"NaN or Inf gradient for {name}")
                  break
            optimizer.step()
            print("loss", loss)
